...
module.exports.getConfig = async function getConfig({
  ...
  try {
    ...
    if (!repoConfig ) {
      fs.readFile('./.github/release-model.json', { encoding: 'utf-8' }, function (err, data) {
        if (!err) {
          repoConf = JSON.parse(data)
          console.log('received data: ' + repoConf)
        }
        else {
          console.log(err)
        }
    ...
    console.log("repoConfig at config js=>", repoConf)
    const config = validateSchema(app, context, repoConf)
    return config
  } catch (error) {
